<?php
class ST_Importer extends ST_WP_Importer {
	var $preStringOption;
	var $results;
	var $getOptions;
	var $saveOptions;
	...
	function process_posts_stepped( $step_number = 1 ) {
		...
		foreach ( $current_posts as $post ) {
            ...
            $result_data[] = ! empty( $post['post_title'] ) ? $post['post_title'] : '';

			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( __( 'Failed to import "%s": Invalid post type %s', 'saturnthemes' ), esc_html( $post['post_title'] ), esc_html( $post['post_type'] ) );
				echo '<br />';
		...
		return array( $step_number, $number_of_posts, $result_data );
	}
