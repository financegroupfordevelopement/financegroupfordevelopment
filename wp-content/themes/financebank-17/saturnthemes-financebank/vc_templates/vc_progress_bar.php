...
$class_to_filter = 'vc_progress_bar saturnthemes-progress-bar wpb_content_element';
...
foreach ( $graph_lines_data as $line ) {
	$unit = ( '' !== $units ) ? ' <span class="vc_label_units primary-background-color">' . $line['value'] . $units . '</span>' : '';
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
			' vc_progress-bar-color-' . $line['color'] : '' )
		. '">';
	$output .= '<small class="vc_label primary-color"' . $line['txtcolor'] . '>' . $line['label'] . '</small>';
	if ( $max_value > 100.00 ) {
