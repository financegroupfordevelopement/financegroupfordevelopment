...
if ( 'none' != $read_more ) {
	...
	$link_end_html = '</a>';
	if ( 'more' == $read_more ) {
		$link_end_html = ' <i class="fa fa-chevron-right"></i>' . $link_end_html;
	}
}
...
if ( $bg_img_hover ) {
	$bg_img_hover_url = wpb_getImageBySize( array(
		'attach_id' => $bg_img_hover,
		'thumb_size' => 'full',
	) );

	$bg_img_hover_url = $bg_img_hover_url['p_img_large'][0];
}
?>
<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" style="background-color: <?php echo esc_attr( $box_bg_color ); ?>;">
	...
	<div class="icon-box-overlay" style="<?php if ( ! empty( $bg_img_hover_url ) ) : ?>background-image: url('<?php echo esc_url( $bg_img_hover_url ); ?>');<?php endif; ?>"></div>
	...
	<?php if ( 'more' == $read_more ) {
		echo '<div class="icon-box-read-more primary-background-color">' . $link_open_html . $read_text . $link_end_html . '</div>';
	} ?>
