...
class saturnthemes_bread_crumb {
	...
	public function saturnthemes_financebank_bread_crumb( $args = '' ) {

		$default = array(
			'type'             => 'list',
			'home_label'       => __( 'Home', 'saturnthemes-financebank' ),
			'search_label'     => __( 'Search Result of "%s"', 'saturnthemes-financebank' ),
			'404_label'        => __( '404 Not Found', 'saturnthemes-financebank' ),
			'category_label'   => _x( '%s', 'category label', 'saturnthemes-financebank' ),
			'tag_label'        => _x( '%s', 'tag label', 'saturnthemes-financebank' ),
			'taxonomy_label'   => _x( '%s', 'taxonomy label', 'saturnthemes-financebank' ),
			'author_label'     => _x( '%s', 'author label', 'saturnthemes-financebank' ),
			'attachment_label' => _x( '%s', 'attachment label', 'saturnthemes-financebank' ),
			'year_label'       => _x( '%s', 'year label', 'saturnthemes-financebank' ),
			'month_label'      => _x( '%s', 'month label', 'saturnthemes-financebank' ),
			'day_label'        => _x( '%s', 'day label', 'saturnthemes-financebank' ),
			'post_type_label'  => _x( '%s', 'post type label', 'saturnthemes-financebank' ),
			'joint_string'     => __( ' &gt; ', 'saturnthemes-financebank' ),
			'navi_element'     => '',
			'elm_class'        => 'saturnthemes_financebank_bread_crumb',
			'elm_id'           => '',
			'li_class'         => '',
			'class_prefix'     => '',
		...
		$default = apply_filters( 'saturnthemes_financebank_bread_crumb_default', $default );
		...
		$saturnthemes_financebank_bread_crumb_arr = $this->get_saturnthemes_financebank_bread_crumb_array( $args );
		...
		if ( $args['type'] == 'string' ) {
			$output = array();
			$cnt    = 1;
			foreach ( $saturnthemes_financebank_bread_crumb_arr as $ancestor ) {
				if ( $cnt == count( $saturnthemes_financebank_bread_crumb_arr ) ) {
			...
			$output = apply_filters( 'saturnthemes_financebank_bread_crumb_after', $output, $args );
			...
			$output = apply_filters( 'saturnthemes_financebank_bread_crumb_before', $output, $args );
		...
		$output = apply_filters( 'saturnthemes_financebank_bread_crumb', $output, $args );
	...
	private function get_saturnthemes_financebank_bread_crumb_array( $args ) {
		global $post;

		$saturnthemes_financebank_bread_crumb_arr   = array();
		$saturnthemes_financebank_bread_crumb_arr[] = array( 'title' => $args['home_label'], 'link' => home_url() . '/' );
		$saturnthemes_financebank_bread_crumb_arr   = $this->add_posts_page_array( $saturnthemes_financebank_bread_crumb_arr );
		if ( is_404() ) {
			$saturnthemes_financebank_bread_crumb_arr[] = array( 'title' => $args['404_label'], 'link' => false );
		} elseif ( is_search() ) {
			$saturnthemes_financebank_bread_crumb_arr[] = array(
			...
			if ( $post->post_parent ) {
				if ( $parent_post = get_post( $post->post_parent ) ) {
					$singular_saturnthemes_financebank_bread_crumb_arr = $this->get_singular_saturnthemes_financebank_bread_crumb_array( $parent_post, $args );
					$saturnthemes_financebank_bread_crumb_arr          = array_merge( $saturnthemes_financebank_bread_crumb_arr, $singular_saturnthemes_financebank_bread_crumb_arr );
				}
			}
			$saturnthemes_financebank_bread_crumb_arr[] = array(
			...
			$singular_saturnthemes_financebank_bread_crumb_arr = $this->get_singular_saturnthemes_financebank_bread_crumb_array( $post, $args );
			$saturnthemes_financebank_bread_crumb_arr          = array_merge( $saturnthemes_financebank_bread_crumb_arr, $singular_saturnthemes_financebank_bread_crumb_arr );
			$saturnthemes_financebank_bread_crumb_arr[]        = array( 'title' => $post->post_title, 'link' => get_permalink( $post->ID ) );
		} elseif ( is_category() ) {
			global $cat;
		...
		return apply_filters( 'saturnthemes_financebank_bread_crumb_arr', $saturnthemes_financebank_bread_crumb_arr, $args );
	}


	private function get_singular_saturnthemes_financebank_bread_crumb_array( $post, $args ) {
		$saturnthemes_financebank_bread_crumb_arr = array();
		...
		return $saturnthemes_financebank_bread_crumb_arr;
	}


	private function add_posts_page_array( $saturnthemes_financebank_bread_crumb_arr ) {
...
if ( ! function_exists( 'saturnthemes_financebank_bread_crumb' ) ) {
	function saturnthemes_financebank_bread_crumb( $args = '' ) {
		global $saturnthemes_bread_crumb;

		return $saturnthemes_bread_crumb->saturnthemes_financebank_bread_crumb( $args );
	}
}